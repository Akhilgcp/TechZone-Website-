...
function GalleryScene({
    ...
    // Safety check: ensure we have valid images before loading textures
    const imageUrls = useMemo(() => {
        const urls = normalizedImages.map((img) => img.src).filter(Boolean);
        return urls.length > 0 ? urls : ['/placeholder.svg']; // Fallback to prevent empty array
    }, [normalizedImages]);
    ...
    const textures = useMemo(() => {
        return Array.isArray(loadedTextures) ? loadedTextures : [loadedTextures];
    }, [loadedTextures]);
            ...
            {planesData.current.map((plane, i) => {
                ...
                return (
                    <ImagePlane
                        ...
                        scale={scale}
                        material={material}
                        onClick={() => onMediaClick?.(plane.imageIndex)}
                    />
                );
            })}
        </>
