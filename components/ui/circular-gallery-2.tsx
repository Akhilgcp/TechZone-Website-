...
class Media {
    ...
    texture!: Texture;
    hasUploadedTexture: boolean = false;
    ...
    constructor({
        ...
        this.font = font;
        // URI encode the path to handle special characters and spaces
        this.image = encodeURI(image);
        this.isVideo = type === 'video' || /\.(mp4|webm|ogg|mov)$/i.test(this.image);
        this.createShader();
        this.createMesh();
        this.createTitle();
        this.onResize();
    ...
    createShader() {
        this.texture = new Texture(this.gl, {
            ...
            minFilter: this.isVideo ? this.gl.LINEAR : this.gl.LINEAR_MIPMAP_LINEAR,
      ...
      `,
        ...
        if (this.isVideo) {
            ...
            this.video.onseeked = () => {
                // Video has seeked to the thumbnail frame, marking it for texture update
                if (this.video && this.video.readyState >= 2) {
                    this.texture.needsUpdate = true;
                    this.hasUploadedTexture = true;
                }
            };
    ...
    update(
        ...
        if (this.isVideo && this.video && !this.hasUploadedTexture && this.video.readyState >= 2) {
            try {
                this.texture.image = this.video;
                this.texture.needsUpdate = true;
                this.hasUploadedTexture = true;
            } catch (e) {
                console.warn("Video texture upload deferred", e);
            }
        }
...
class App {
    ...
    onTouchUp(e: MouseEvent | TouchEvent) {
        ...
        if (distance < 5 && this.onItemClick) {
            // Click/Tap detected
            const clientX = x;
            const clientY = "changedTouches" in e ? e.changedTouches[0].clientY : (e as MouseEvent).clientY;

            // Simple hit test for items
            // Convert click to NDC (-1 to 1)
            const rect = this.container.getBoundingClientRect();
            const ndcX = ((clientX - rect.left) / rect.width) * 2 - 1;
            const ndcY = -((clientY - rect.top) / rect.height) * 2 + 1;

            // Find closest plane to the click
            let closestItem = -1;
            let minDistance = Float32Array.from([Infinity])[0];

            this.medias.forEach((media, index) => {
                // Get plane position in screen space (simplified)
                // Since it's a 2D-ish layout with arc, we can check x/y
                // This is a rough estimation but works for this gallery
                const planeX = (media.plane.position.x / this.viewport.width) * 2;
                const planeY = (media.plane.position.y / this.viewport.height) * 2;

                const dist = Math.sqrt(Math.pow(ndcX - planeX, 2) + Math.pow(ndcY - planeY, 2));
                if (dist < 0.4 && dist < minDistance) {
                    minDistance = dist;
                    closestItem = index % (this.medias.length / 2);
                }
            });
